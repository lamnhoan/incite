		...
		chunks: []chunkPlan{
			{
				...
				startQueryErrs: []error{
					&types.InvalidParameterException{Message: sp("terrible query writing there bud")},
				},
				startQuerySuccess: false,
			},
		},
		err: &StartQueryError{"a poorly written query", defaultStart, defaultEnd, &types.InvalidParameterException{Message: sp("terrible query writing there bud")}},
		stats: Stats{
		...
		chunks: []chunkPlan{
			{
				...
				startQueryErrs: []error{
					&types.LimitExceededException{Message: sp("use less")},
					&smithy.GenericAPIError{Code: "Throttling", Message: "slow down"},
					&types.ServiceUnavailableException{Message: sp("wait for it...")},
					io.EOF,
				},
				startQuerySuccess: true,
				pollOutputs: []chunkPollOutput{
					...
					{
						err: &types.ServiceUnavailableException{Message: sp("a blip in service")},
					},
					{
						status: types.QueryStatusRunning,
						results: []Result{
					...
					{
						err: &smithy.GenericAPIError{Code: "throttling has occurred", Message: "and you were the recipient of the throttling"},
					},
					{
						status: types.QueryStatusComplete,
						results: []Result{
		...
		chunks: []chunkPlan{
			{
				...
				startQuerySuccess: true,
				startQueryErrs: []error{
					&types.LimitExceededException{Message: sp("truckin', got my chips cashed in")},
					&types.LimitExceededException{Message: sp("like the do-dah man")},
					&types.LimitExceededException{Message: sp("together, more or less in line")},
					&types.LimitExceededException{Message: sp("just keep truckin' on")},
				},
				pollOutputs: []chunkPollOutput{
					{
						status: types.QueryStatusComplete,
			...
			},
			{
				...
				startQuerySuccess: true,
				startQueryErrs: []error{
					&types.LimitExceededException{Message: sp("arrows of neon and flashing marquees out on main street")},
					&types.LimitExceededException{Message: sp("chicago, new york, detroit")},
					&types.LimitExceededException{Message: sp("and it's all on the same street")},
					&types.LimitExceededException{Message: sp("your typical city involved in a typical day dream")},
					&types.LimitExceededException{Message: sp("hang it up and see what tomorrow brings")},
				},
				pollOutputs: []chunkPollOutput{
					{
						results: []Result{
	...
	//   chunk produces stats MaxLimit+1+1+1+1/MaxLimit+1+1+1+1/MaxLimit+1+1+1+1.
	{
		...
		chunks: []chunkPlan{
			// CHUNK 0.
			{
				...
				startQuerySuccess: true,
				startQueryErrs:    []error{issue13Error(), io.EOF, cwlErr("ServiceUnavailableException", "pending", issue13Error())},
				pollOutputs: []chunkPollOutput{
					{
						status: types.QueryStatusScheduled,
					},
			...
			// CHUNK 5.
			{
				...
				startQuerySuccess: true,
				startQueryErrs:    []error{&smithy.GenericAPIError{Code: "connection reset", Message: "reset that connection", Fault: smithy.FaultUnknown}},
				pollOutputs: []chunkPollOutput{
					{
			...
			// CHUNK 10.
			{
				...
				startQuerySuccess: true,
				startQueryErrs:    []error{issue13Error(), io.EOF, &types.ServiceUnavailableException{Message: sp("pending")}},
				pollOutputs: []chunkPollOutput{
					{
						status: types.QueryStatusScheduled,
					},
			...
			// CHUNK 20 [split sub-chunk 1/4].
			{
				...
				startQueryErrs:    []error{&types.LimitExceededException{Message: sp("y'all have too many dang queries goin' on..., nil")}},
				startQuerySuccess: true,
				pollOutputs: []chunkPollOutput{
					{
						results: maxLimitResults[0 : MaxLimit/4],
...
func statusp(s types.QueryStatus) types.QueryStatus {
	return s
}
...
func getQueryResultsOutput(r []Result, status string, stats *Stats) *cloudwatchlogs.GetQueryResultsOutput {
	...
	if status != "" {
		o.Status = types.QueryStatus(status)
	}
	if stats != nil {
